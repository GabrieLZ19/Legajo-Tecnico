...
interface PreguntaForm {
  pregunta: string;
  opciones: string[];
  respuesta_correcta: number | number[]; // Index o array de índices
  es_multiple?: boolean;
}
...
export default function NuevaCapacitacionPage() {
  ...
  const toggleEsMultiple = (idx: number) => {
    ...
    // Resetear respuesta correcta según el tipo
  ...
  const toggleRespuestaCorrectaMultiple = (pregIdx: number, optIdx: number) => {
    ...
    if (current.includes(optIdx)) {
      // Dejar al menos un elemento si es posible
  ...
  const eliminarOpcion = (pregIdx: number, optIdx: number) => {
    ...
    // Ajustar selección correcta si se borra el índice
    ...
    const plantillas = {
      ...
      incendios: [
        ...
        },
        {
          ...
          respuesta_correcta: [0, 1], // EJEMPLO CON RESPUESTA MÚLTIPLE CORRECTA
          es_multiple: true,
        },
        {
          ...
          {preguntas.map((p, idx) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                  {/* Toggle para Múltiples Respuestas Correctas */}

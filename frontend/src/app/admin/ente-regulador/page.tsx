...
export default function EnteReguladorPage() {
  const { getArchivoData } = useArchivo();
    ...
    return (
      <div className="space-y-6 animate-pulse">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-40 bg-slate-200 rounded-4xl"></div>
          ))}
        </div>
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Warning Banner */}
      <div className="bg-[#FFF9E6] border border-[#FFE4A3] rounded-3xl p-6 flex items-start gap-4">
        ...
        <div>
          ...
          <p className="text-sm font-medium text-[#B45309] opacity-80 mt-0.5">
            El alcance está limitado a la información habilitada por el
      ...
      <div className="bg-white rounded-4xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-8">
          <h2 className="text-xl font-bold text-slate-900">
            Informes habilitados para consulta
          </h2>
        </div>
...
function StatCard({
  ...
  return (
    <div className="bg-white rounded-4xl p-8 shadow-sm border border-slate-100 space-y-6">
      <div
        className={`h-12 w-12 ${iconBg} rounded-2xl flex items-center justify-center`}
      >
        {icon}
      </div>
      <div>

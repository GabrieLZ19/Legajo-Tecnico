...
export default function EnteArchivoPage() {
  ...
  useEffect(() => {
    api.get("/ente/archivo").then((res) => setDocumentos(res.data.documentos || [])).catch(console.error);
  }, []);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">Informes, capacitaciones y EPP habilitados.</p>
      </div>
      <div className="bg-white rounded-3xl border divide-y">
        {documentos.length === 0 ? (
          <p className="p-8 text-sm text-slate-400 font-semibold">Sin documentos habilitados.</p>
        ) : (
          documentos.map((doc) => (
            <div key={`${doc.tipo}-${doc.id}`} className="px-5 py-4 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-[11px] text-slate-400">
                  {doc.empresa_razon_social} · {new Date(doc.fecha).toLocaleDateString("es-AR")}
                </p>
              </div>
              {doc.pdf_disponible && (
                <button
